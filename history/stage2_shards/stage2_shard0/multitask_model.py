...
# 添加OpenFly-Platform到路径（自动探测）
OPENFLY_TRAIN_CANDIDATES = [
    os.environ.get("OPENFLY_TRAIN", ""),
    "/home/ubuntu/data1/zx/1OpenFly-Platform/OpenFly-Platform/train",
    "/home/ubuntu/data1/zx/OpenFly-Platform/train",
    "/home/ubuntu/data1/lyy/OpenFly-Platform/train",
]

for _p in OPENFLY_TRAIN_CANDIDATES:
    if _p and os.path.isdir(os.path.join(_p, "extern", "hf")):
        if _p not in sys.path:
            sys.path.insert(0, _p)
        print(f"[MultiTaskVLA] use OpenFly train path: {_p}")
        break
...
OPENFLY_PATH = None
openfly_train_path = None


_OPENFLY_CANDIDATES = [
    os.environ.get("OPENFLY_PATH", ""),
    "/home/ubuntu/data1/zx/1OpenFly-Platform/OpenFly-Platform",
    "/home/ubuntu/data1/zx/OpenFly-Platform",
    "/home/ubuntu/data1/lyy/OpenFly-Platform",
    "/home/ubuntu/data1/wrp/OpenFly-Platform",
]

for _base in _OPENFLY_CANDIDATES:
    if not _base:
        continue
    _base = _base.rstrip("/")
    _p1 = os.path.join(_base, "train", "extern", "hf", "configuration_prismatic.py")
    _p2 = os.path.join(_base, "extern", "hf", "configuration_prismatic.py")
    if os.path.isfile(_p1):
        OPENFLY_PATH = _base
        openfly_train_path = os.path.join(_base, "train")
        break
    if os.path.isfile(_p2):
        OPENFLY_PATH = _base
        openfly_train_path = _base
        break

if openfly_train_path is None:
    OPENFLY_PATH = "/home/ubuntu/data1/zx/1OpenFly-Platform/OpenFly-Platform"
    openfly_train_path = os.path.join(OPENFLY_PATH, "train")

if openfly_train_path not in sys.path:
    sys.path.insert(0, openfly_train_path)

print(f"[MultiTaskVLA] OPENFLY_PATH={OPENFLY_PATH}")
print(f"[MultiTaskVLA] OPENFLY_TRAIN_PATH={openfly_train_path}")
...
class MultiTaskVLA(nn.Module):
    ...
    def __init__(
        ...
        # 1. 检查OpenVLA是否可用
        if not OPENVLA_AVAILABLE and self.verbose:
            print("⚠ OpenVLA模块未导入成功，继续走OpenFly原生加载分支")
